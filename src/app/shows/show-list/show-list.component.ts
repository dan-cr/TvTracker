...
export class ShowListComponent implements OnInit {
	...
	/**
	 * Get show results by search term
	 * @param {} page
	 * @param {} term
	 */
	searchShowsByName(page, term) {
		this.tmdb.getShowsByName(page, term).pipe(
			...
			// Swap Genre IDs for genre names
			this.assignShowGenreNames(gens.genres);
		})
	}
	...
	/**
	 * Assign comma delimited string of genre names to each show object which have been mapped by the genre id
	 */
	public assignShowGenreNames(genres) {
		if (!this.shows && !this.genres) return;

		// Genre Map lookup table
		const genreLookup = new Map();
		genres.forEach(genre => genreLookup.set(genre.id, genre));
	...
	changeShowCategory(e) {
		...
		// Reset URL to default state in case user has submitted the search form
	...
	/**
	 * Calculate page range from current page, total and range, then return an array of page numbers
	 * @param  {} currPage
	 * @param  {} totalPages
	 * @param  {} range
	 */
